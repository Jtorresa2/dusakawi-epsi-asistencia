// @ts-ignore - no @types/pdfkit installed
...
// ─── Helpers ─────────────────────────────────────
function _triangulo(doc: any, x: number, y: number, size: number, color: string, abajo: boolean) {
  doc.save();
  doc.fillColor(color);
...
function _rombo(doc: any, x: number, y: number, size: number, color: string) {
  doc.save();
  const pts = [
    [x + size / 2, y],
    [x + size, y + size / 2],
...
// ─── drawContent ──────────────────────────────────
function drawContent(doc: any) {
  doc.x = BODY_X;
  doc.y = BODY_Y;
}

interface MembreteMeta {
  codigo?: unknown;
  version?: unknown;
  emision?: unknown;
  vigencia?: unknown;
...
// ─── generarMembrete ──────────────────────────────
export function generarMembrete(res: Response, metadata?: MembreteMeta, callback?: (doc: any) => void) {
...
export function generarPlantillaIncidencia(doc: any, incidencia: IncidenciaData) {
  const PAGE_W = 595.28;
  const BODY_X = 65;
  ...
  doc.text(`ASUNTO: ${String(incidencia.tipo ?? "").toUpperCase() || "—"}`);
  doc.moveDown(0.3);
  ...
  const fields = [
    ...
    `Fecha:     ${incidencia.fecha ? new Date(String(incidencia.fecha)).toLocaleDateString("es-CO") : "—"}`,
  ];
  ...
  if (incidencia.evidencia_url) {
    const evPath = path.join(__dirname, "../../", String(incidencia.evidencia_url).replace(/^\//, ""));

...
exports.obtenerTodos = async (activo = null) => {
  ...
  const params = [];
  if (activo !== null) { sql += " AND activo = $1"; params.push(activo); }
...
exports.obtenerPorId = async (id) => {
  const { rows } = await pool.query("SELECT * FROM festivos WHERE id = $1", [id]);
  return rows[0] || null;
};
...
exports.crear = async ({ fecha, nombre, tipo }) => {
  ...
  const { rows: [result] } = await pool.query(
    `INSERT INTO festivos (fecha, nombre, tipo) VALUES ($1, $2, $3) RETURNING id`,
    [fecha, nombre, tipoValido]
  );
  return { id: result?.id ?? 0, fecha, nombre, tipo: tipoValido };
};

exports.actualizar = async (id, { fecha, nombre, tipo, activo }) => {
  const campos = [];
  const params = [];
  if (fecha !== undefined) { campos.push("fecha = $1"); params.push(fecha); }
  if (nombre !== undefined) { campos.push(`nombre = $${params.length + 1}`); params.push(nombre); }
  if (tipo !== undefined) { campos.push(`tipo = $${params.length + 1}`); params.push(tipo); }
  if (activo !== undefined) { campos.push(`activo = $${params.length + 1}`); params.push(activo); }
  if (campos.length === 0) return { id };
  params.push(id);
  await pool.query(`UPDATE festivos SET ${campos.join(", ")} WHERE id = $${params.length}`, params);
  return { id };
};

exports.eliminar = async (id) => {
  await pool.query("DELETE FROM festivos WHERE id = $1", [id]);
  return { id };
};

exports.verificarFestivo = async (fecha) => {
  const { rows } = await pool.query(
    "SELECT id, nombre, tipo FROM festivos WHERE fecha = $1 AND activo = TRUE",
    [fecha]
  );
  return rows[0] || null;
};
...
exports.generarNacionales = async (year) => {
  ...
  for (const festivo of lista) {
    const { rows } = await pool.query(
      "SELECT id FROM festivos WHERE fecha = $1 AND tipo = 'nacional'",
      [festivo.fecha]
    );
    if (rows.length === 0) {
      await pool.query(
        "INSERT INTO festivos (fecha, nombre, tipo) VALUES ($1, $2, 'nacional') RETURNING id",
